...
namespace caffe {
	...
	void BatchReNormLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
		...
		bool first_iter_in_step = (iter%iter_size_ == 0);   // se iter_size == 1, sempre true. Altrimenti la prima iter del blocchetto
		...
		if (!use_global_stats_) {
			...
			{
				// predispone diff (gradiente) per passo back
			...
			Dtype moving_average_fraction = first_iter_in_step ? moving_average_fraction_ : 1;   // sempre moving_average_fraction_ per iter_size = 1
		...
		{
			Dtype cur_r_max = std::max((Dtype)1, std::min(1 + (step - step_to_init_ + 1)*(r_max_ - 1) / (step_to_r_max_ - step_to_init_), r_max_));
			Dtype cur_r_min = 1. / cur_r_max;
			Dtype cur_d_max = std::max((Dtype)0, std::min((step - step_to_init_ + 1)*d_max_ / (step_to_d_max_ - step_to_init_), d_max_));
			...
			{
				r_.mutable_cpu_data()[i] = std::min(cur_r_max, std::max(r_.mutable_cpu_data()[i], cur_r_min));
				d_.mutable_cpu_data()[i] = std::min(cur_d_max, std::max(d_.mutable_cpu_data()[i], cur_d_min));
			}

			// moltiplica per r (vedi paper)
			caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, num, channels_, 1, 1,
