...
namespace caffe {
	...
	void BatchReNormLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
		...
		{
			Dtype cur_r_max = max((Dtype)1, min(1 + (step - step_to_init_ + 1)*(r_max_ - 1) / (step_to_r_max_ - step_to_init_), r_max_));
			Dtype cur_r_min = 1. / cur_r_max;
			Dtype cur_d_max = max((Dtype)0, min((step - step_to_init_ + 1)*d_max_ / (step_to_d_max_ - step_to_init_), d_max_));
